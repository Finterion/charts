/**
 * JSON Schema (Draft 2020-12) for Finterion ChartSpec v1.
 *
 * Hand-written so it can be:
 *   - shipped as a `tool`/`function` schema for LLM structured output
 *     (OpenAI tools, Anthropic tool use, Gemini function calling)
 *   - used by ajv / @cfworker/json-schema for runtime validation
 *   - referenced from forum docs (`$schema`)
 */
...
export const CHART_SPEC_SCHEMA = {
  ...
  $defs: {
    formatDirective: {
      enum: ['pct0', 'pct1', 'pct2', 'fixed0', 'fixed1', 'fixed2', 'short-num', 'iso-date'],
    },
          ...
          allOf: [
            { $ref: '#/$defs/panelBase' },
            {
              ...
              properties: {
                ...
                overlays: { type: 'array', items: { $ref: '#/$defs/indicatorSeries' } },
              },
            },
          ],
        },
